...
//验证bookinLogin.tpl 的登录表单
function formIsEmptyforMedicalCard(form){
	if(form.user_name.value=="医保卡号码"  || form.user_name.value.replace(/\s/g,"")=="" ){
		alert("医保卡号码不能为空");
		return false;
	}
	if(form.password.value.replace(/\s/g,"")=="" || form.password.value=="您的医保卡登记姓名"){
		alert("您的医保卡登记姓名不能为空");
		return false;
	}
	if(form.code.value.replace(/\s/g,"")==""){
		alert("验证码格式错误");
		return false;
	}
	return true;
...
//验证康众服务登录表单kzServiceLogin.tpl.php
function formIsEmptyForKzServiceLogin(form) 
{
	var userName = document.getElementsByName("user_name")[1].value;
	if(userName=="卡号/会员名"  || userName.replace(/\s/g,"")=="" || userName=="卡号/手机号/会员名"){
		alert("用户名不能为空");
		return false;
	}
	if(document.getElementsByName("password")[1].value.replace(/\s/g,"")==""){
		alert("密码不能为空");
		return false;
	}
	if(document.getElementsByName("code")[1].value.replace(/\s/g,"")==""){
		alert("验证码格式错误");
		return false;
	}
	return true;
